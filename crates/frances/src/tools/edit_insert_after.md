...
Args: `{ path, anchor, text }`
  ...
  anchor:  full rendered anchor line that the new content will be inserted AFTER — `Word§content`, exactly as `read_file` produced it.
...
To insert a docstring before `goodbye` (which is the same as inserting AFTER the blank line — but if there isn't one, you can pin to `Banana` and insert after that, or use `edit_insert_before` on `Daisy`). To add a comment after the print:
