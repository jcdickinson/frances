...
If the regex matches zero times, the call succeeds with a no-changes result; it does not write the file .

On a real change, the file is written through the normal edit pipeline: the file is re-read and re-anchored, and the returned content is the same kind of diff block produced by `file_replace_lines`.
