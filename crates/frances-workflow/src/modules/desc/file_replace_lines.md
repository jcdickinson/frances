...
Anchor protocol: every line in a `file_read` (or post-edit diff) is rendered as `Word§content`. The `Word` half identifies the line by anchor; the content half is what's currently on that line. Pass back both halves verbatim — the engine splits on the first `§`, validates the anchor word against the cached file, and compares the content (trimmed) for safety. On a content mismatch the call fails and you should re-read the file before retrying. Each of `anchor`/`end_anchor` is a single rendered line: to span a range, put the first line in `anchor` and the last in `end_anchor` — never glue several `Word§content` lines into one field. A newline in an anchor field is rejected.
