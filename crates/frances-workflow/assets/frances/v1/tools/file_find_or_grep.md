...
Result entries are sorted alphabetically by path. Each entry is `{ path, size, mtime, binary, match_count?, first_match? }`. Matching-line text is limited to a 512-byte excerpt around the match; oversized lines set `first_match.line_bytes` to the original byte length. The structured result is capped at 1000 entries, and the inline response is capped at 16 KiB. Either limit reports its omission explicitly so I know to narrow the query.
