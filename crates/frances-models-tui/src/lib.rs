//! Shared TUI-side wire vocabulary.
//!
...
//! `frances-tui` (consumer via the section dispatcher) depend on
//! this crate.
//!
//! What lives here:
//!
...
//!   etc.); the TUI dispatches `make_section(kind)` on first appearance
//!   of a new section id.
...
//! - [`ShellState`] / [`ReasoningState`] — terminal-status enums carried
//!   inside the matching [`SectionKind`] variants.
//! - [`WireSectionEvent`] — what flows through the runtime → TUI
...
/// assigned by `transcript.push` on the workflow side. The TUI uses it
...
/// What flows through the runtime → TUI channel. Self-describing:
...
/// with an unseen id implicitly constructs the section (the TUI's
