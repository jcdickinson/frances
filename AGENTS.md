...
- **Never discard an error silently.** If an error is intentionally ignored for
  any reason, emit it through `tracing` first. This applies regardless of the
  syntax used; common forms include:

  ```rust
  operation()
      .inspect_err(|error| tracing::warn!(?error, "operation failed"))
      .ok();
  ...
  Error parameters recorded by `tracing` must always be named `error`, never
  `err`, `e`, `source`, or another alias. Choose the tracing level carefully:
  do not spam `tracing::error!`. Reserve `error!` for failures that genuinely
  require attention or recovery from the user; most discarded operational
  failures belong at `warn!` or a lower level.
